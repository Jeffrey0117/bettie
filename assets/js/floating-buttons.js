...
(function() {
    ...
    var template = '\
        <div class="fixed-buttons flex items-center flex-col fixed z-50 md:right-[4rem] right-[2rem] md:bottom-[4rem] bottom-[2rem]">\
            <a href="' + CONFIG.reserveLink + '"\
                class="reserve-btn group size-[4.5rem] bg-[#AF913B] hover:bg-[#9a7e33] transition-all duration-300 flex items-center justify-center rounded-full border border-[#AF913B] hover:border-[#9a7e33]">\
                <span class="text-white text-[1.2rem] font-bold">\u9810\u7D04</span>\
            </a>\
            <button id="service-toggle-btn"\
                class="service-btn group md:mt-[1.5rem] mt-[0.8rem] size-[4.5rem] bg-black hover:bg-[#AF913B] transition-all duration-300 flex items-center justify-center rounded-full border border-black hover:border-[#AF913B] cursor-pointer"\
            ...
            <div id="service-list" class="service-list">\
                <a href="' + CONFIG.igLink + '" target="_blank"\
                    class="group md:mt-[1.5rem] mt-[0.8rem] size-[4.5rem] bg-black hover:bg-[#AF913B] transition-all duration-300 flex items-center justify-center rounded-full border border-black hover:border-[#AF913B]">\
            ...
            <button class="group md:mt-[1.5rem] mt-[0.8rem] got_to_top overflow-hidden cursor-pointer size-[4.5rem] bg-white hover:bg-[#AF913B] transition-all duration-300 flex items-center justify-center rounded-full border border-[#AF913B]">\
                <svg class="w-[1.6rem]" width="16" height="8" viewBox="0 0 16 8" fill="none" xmlns="http://www.w3.org/2000/svg">\
                    <path d="M15 7L8 1L0.999999 7" stroke="#AF913B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="stroke-[#AF913B] group-hover:stroke-white transition-colors duration-300" />\
                </svg>\
            </button>\
        </div>\
    ';
    ...
    function init() {
        // Remove existing inline fixed buttons if present
        var existingFixed = document.querySelector('.flex.items-center.flex-col.fixed.z-50');
        if (existingFixed && !existingFixed.classList.contains('fixed-buttons')) {
